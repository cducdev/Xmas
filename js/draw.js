...
drawBtn.addEventListener("click", async () => {
	const userType = box.children[0].value;
	try {
		const data = await fetchApi(userType);
		for (let i = 0; i < inputNameBox.children.length; i++)
			inputNameBox.children[i].style.display = "none";
		...
		const innerT = data.msg.replace("bạn", userName);
		setTimeout(() => {
			inputNameBox.innerHTML += `
				<p class="white-text" style = "width: 90%; margin: 10px auto;">${innerT}</p>
			`;
		}, 8000);
	} catch (e) {
		console.log(e);
